...
public:
    ...
    status_t setPreviewWindow(const unsigned int hlay)
    {
...
#ifdef ADAS_ENABLE
	void setAdasCallbacks(adas_data_callback adas_data_cb)
	{
		mAdasDataCB = adas_data_cb;
		if (mDevice->ops->set_adas_callbacks) {
			mDevice->ops->set_adas_callbacks(mDevice, __adas_data_cb);
		}
	}

	status_t setAdasParameters(int key,int value)
	{
		ALOGV("%s(%d:%d)",__FUNCTION__, key,value);
		if(mDevice->ops->set_adas_parameters)
			return mDevice->ops->set_adas_parameters(mDevice, key,value);
		return INVALID_OPERATION;
	}
#endif
    /**
	...
	{
		if(mDevice->ops->set_analog_input_color)
			mDevice->ops->set_analog_input_color(mDevice,brightness,contrast,saturation);
	}
    /**
     ...
     * frames.
     */
    void releaseRecordingFrame(const char* mem)
    {
        //ALOGV("%s(%s)", __FUNCTION__, mName.string());
        if (mDevice->ops->release_recording_frame) {
			...
			#endif
            return mDevice->ops->release_recording_frame(mDevice, mem);
        }
    }
    ...
    {
        ALOGV("%s~~~~~~~(%s),dispMode(%d)", __FUNCTION__, mWaterMark.string(),dispMode);
        if(mDevice->ops->set_waterMarkMultiple)
        {
            status_t ret = mDevice->ops->set_waterMarkMultiple(mDevice, mWaterMark.string(), dispMode);
    ...
    /** for flip setting */
    status_t setCameraFlipStatus(int flipEnable)
    {
        ALOGV("%s : flipEnable = %d",__FUNCTION__,flipEnable);
        if (mDevice->ops->set_CameraFlipStatus)
            return mDevice->ops->set_CameraFlipStatus(mDevice, flipEnable);
        return INVALID_OPERATION;
    }
	...
	status_t setPreviewParam( struct view_info pvi,struct src_info  psi,bool tvout,int tvch)
	{
		...
		if(mDevice->ops->set_previewparam) {
			return mDevice->ops->set_previewparam(mDevice,pvi,psi,tvout,tvch);
		}
		return INVALID_OPERATION; 
	}
...
private:
                          ...
                          void *user)
    {
		...
		pmetadata.number_of_faces=mem->mBufSize*mem->mNumBufs;
        __this->mDataCb(msg_type,(char *) mem->mBuffers[index], &pmetadata, __this->mCbUser);
    }
	...
	if(mem == NULL) {
		ALOGE("camera heap memory is NULL");
		return;
	}
        if (index >= mem->mNumBufs) {
            ALOGE("%s: invalid buffer index %d, max allowed is %d", __FUNCTION__,
        ...
        __this->mAdasDataCB(msg_type, (char *)mem->mBuffers[index], metadata, __this->mCbUser);
	}
#endif
                             ...
                             void *user)
    {
        ...
        __this->mDataCbTimestamp(timestamp, msg_type, (char *)mem->mBuffers[index], __this->mCbUser);
    }
    ...
    public:
                         ...
                         mNumBufs(num_buffers)
        {
            ...
            mHeap = (char*)malloc(buf_size * num_buffers);
			if(mHeap==NULL)
			{
        ...
        void commonInitialization()
        {
            ...
            for (uint_t i = 0; i < mNumBufs; i++)
                mBuffers[i] = new MemoryBase(mHeap,
                                             ...
                                             mBufSize);
#else
			mBuffers=new int[mNumBufs];
			for (uint_t i = 0; i < mNumBufs; i++)
				mBuffers[i] = (int)mHeap+ i * mBufSize;
...
#define container_of(ptr, type, member) ({                      \
    ...
    static int __set_buffers_transform(struct preview_stream_ops *w, int transform)
    {
        ANativeWindow *a = anw(w);
        return native_window_set_buffers_transform(a, transform);
    }
    ...
    void initHalPreviewWindow()
    {
        ...
        mHalPreviewWindow.nw.set_buffers_transform =
                __set_buffers_transform;
    }
