...
typedef struct
{
	...
	unsigned int            lcd_edp_tx_ic;   //0:anx9804;  1:anx6345
	unsigned int            lcd_edp_tx_rate; //1(1.62G); 2(2.7G); 3(5.4G)
	unsigned int            lcd_edp_tx_lane; //  1/2/4lane
...
typedef struct
{
	...
	unsigned int	pll_2x;
	unsigned int	hdmi_clk;
	unsigned int	avi_pr;
	unsigned int	hor_pixels;
	unsigned int	ver_pixels;
...
struct sunxi_disp_source_ops
{
  ...
  int (*sunxi_lcd_dsi_io_enable)(unsigned int screen_id);
  int (*sunxi_lcd_dsi_io_disable)(unsigned int screen_id);
...
typedef enum tag_DISP_CMD
{
	...
	DISP_CMD_LAYER_SET_CONFIG =0x47,
