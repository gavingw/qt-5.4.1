...
{
	ALOGD("create timer data is 0x%p\n", data);
    struct sigevent se;
...
int setOneShotTimer(time_t sec, long nsec, timer_t timerid)
{
	...
	ALOGD("begain to set the one shot timer, timer id = 0x%lx\n", (unsigned long)timerid);
...
{
    struct itimerspec ts, ots; 
	ALOGD("begain to set the period timer, timer id is 0x%lx\n", (unsigned long)timerid);	
...
{
	ALOGD("deleting the posix timer, timer id = 0x%lx\n", (unsigned long)timerid);
