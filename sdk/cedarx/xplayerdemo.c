...
int CallbackForAwPlayer(void* pUserData, int msg, int ext1, void* param)
{
    ...
    switch(msg)
    {
        ...
        case AWPLAYER_MEDIA_PLAYBACK_COMPLETE:
        {
            ...
            //* TODO
            g_cmdCnt = 2;
            printf("play complete g_cmdCnt = %d\n",g_cmdCnt);
            break;
        }
...
int main(int argc, char** argv)
{
  ...
  //  CEDARX_UNUSE(argv);
	...
	sprintf(urlbuf,"set url:%s",argv[1]);
	printf("url is %s,strlen=%d\n",urlbuf,strlen(urlbuf));
	...
	if(!layer){
		printf("LayerCreate_DE failed,return now\n");
		return -1;
	}
    SoundCtrl* sound = TinaSoundDeviceInit(44100,2,SND_PCM_FORMAT_S16_LE,"default");

	SubCtrl*   sub  = SubtitleCreate(SubCallbackProcess, topAppSubtitleCb);
	...
	if(!sound){
		...
		TinaSoundDeviceSetCallback(sound, &cbUser, &datUser);
    ...
    if(!di){
		printf("DeinterlaceCreate failed!!!\n");
	}else{
    ...
    while(!bQuit)
    {
		...
		switch(g_cmdCnt){
		case 0:
			memcpy(strCommandLine,urlbuf,strlen(urlbuf)+1);
			g_cmdCnt = 1;
			break;
			
		case 1:
			memcpy(strCommandLine,"play",sizeof("play"));
			g_cmdCnt = 0xff;
			break;
			
		case 2:
			memcpy(strCommandLine,"quit",sizeof("quit"));
			g_cmdCnt = 0xff;
			break;
