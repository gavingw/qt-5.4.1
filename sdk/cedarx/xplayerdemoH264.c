...
static cdx_int32 PrepareThread(CdxStreamT *stream)
{
	...
	#if 1
    FILE* inFp = fopen("/tmp/a.mp4", "rb");
    if(inFp != NULL)
    {
        printf("open /tmp/a.mp4 success\n"); 
        fread(streamingSource->probeData.buf, 1, ProbeDataLen, inFp);
        fclose(inFp);
    }else{
		printf("open /tmp/a.mp4 failed\n");
	}
...
CdxStreamT *StreamingSourceOpen(const char ** source,
