...
int iniparser_init(const char *filename)
{
    ...
    if(!filename) path = INI_PATH;
    else path = filename;

    if ((ini_filep = fopen(path, "r+")) == NULL) {
...
static int confirm_location_of_key(FILE *ini_filep, const char *section, const char *key, int *start)
{
	...
	while (fgets(line, ASCIILINESZ+1, ini_filep) != NULL) {
		...
		if(iniparser_line(line, ini_st.section, ini_st.key, ini_st.val) == LINE_VALUE) {
		    if(!strncmp(ini_st.section, section, strlen(ini_st.section)) 
...
static int get_a_ini_value(const char *section, const char *key, char *val)
{
	...
	FILE * ini_filep = NULL;
#if 0	
	extern int GetChipId(int chipId);
	
	int thisId=3;
	int hwRet=GetChipId(thisId);
	
	if (hwRet==255)
	{
		ALOGV("init cin sucess !\r\n");
	}
	else
	{
		ALOGV("init cin fail !\r\n");
		thisId=5;
		hwRet=GetChipId(thisId);
		  if (hwRet!=255)
			return -1;
	}
#endif
