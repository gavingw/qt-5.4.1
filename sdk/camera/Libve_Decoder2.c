...
void Libve_dec2(VideoDecoder** mVideoDecoder, const void *in, void *out, VideoStreamInfo* pVideoInfo, VideoStreamDataInfo* dataInfo, VConfig* pVconfig,struct ScMemOpsS *memops)
{
    int   ret;
    char* pBuf0;
    char* pBuf1;
    ...
    if(ret == VDECODE_RESULT_FRAME_DECODED || ret == VDECODE_RESULT_KEYFRAME_DECODED)
    {
        ...
        if(pPicture)
        {
			...
			CdcMemFlushCache(memops,(void*)pPicture->pData0,pVideoInfo->nWidth*pVideoInfo->nHeight);
			CdcMemFlushCache(memops,(void*)pPicture->pData1,pVideoInfo->nWidth*pVideoInfo->nHeight/2);
