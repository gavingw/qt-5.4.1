...
status_t CallbackNotifier::adasResultMsg(void *adas)
{
	...
	{
		camera_memory_t *cam_buff = mGetMemoryCB(-1, sizeof(ADASOUT_IF), 1, NULL);
		if (NULL == cam_buff && NULL != cam_buff->data) {
			LOGE("%s: Memory failure in CAMERA_MSG_ADAS_METADATA", __FUNCTION__);
			return NO_MEMORY;
		}
		mAdasDataCB(CAMERA_MSG_ADAS_METADATA, cam_buff, 0, adas, mCallbackCookie);
...
//fengyun add end
	...
	if (isMessageEnabled(CAMERA_MSG_VIDEO_FRAME) && isVideoRecordingEnabled()) 
	{
		...
		//ALOGE("--------------cam_buff=%p  data=%p addVy=%p,vddrVc=%p index=%d",cam_buff,cam_buff->data,pbuf->addrVirY,pbuf->addrVirC,pbuf->index);
        if (NULL != cam_buff && NULL != cam_buff->data)
        {
			if(channel >= 0)
			{
				#ifdef USE_ION_MEM_ALLOCATOR
	            ion_flush_cache((void*)pbuf->addrVirY, pbuf->width * pbuf->height * 3 / 2);
	            #elif USE_CDX_NEW_MEM_API
				if(mCameraMemops!=NULL)
					CdcMemFlushCache(mCameraMemops,(void*)pbuf->addrVirY, pbuf->width * pbuf->height * 3 / 2);
				#else
	            sunxi_flush_cache((void*)pbuf->addrVirY, pbuf->width * pbuf->height * 3 / 2);
	            #endif
	            switch (channel) {
	                case BV_CHANNEL_BIRDVIEW:
		                sInputBuffer.bEnableCorp = 0;
			            sInputBuffer.sCropInfo.nLeft = 0;
			            sInputBuffer.sCropInfo.nTop = 0;
			            sInputBuffer.sCropInfo.nWidth  = BIRDVIEW_OUTPUT_WIDTH;
			            sInputBuffer.sCropInfo.nHeight = BIRDVIEW_OUTPUT_HEIGHT;
		                break;
		            case BV_CHANNEL_FRONT:
		                sInputBuffer.bEnableCorp = 1;
			            sInputBuffer.sCropInfo.nLeft = 0;
			            sInputBuffer.sCropInfo.nTop = 0;
			            sInputBuffer.sCropInfo.nWidth  = BIRDVIEW_SOURCE_SINGLE_WIDTH;
			            sInputBuffer.sCropInfo.nHeight = BIRDVIEW_SOURCE_SINGLE_HEIGHT;
		                break;
		            case BV_CHANNEL_BACK:
		                sInputBuffer.bEnableCorp = 1;
			            sInputBuffer.sCropInfo.nLeft = BIRDVIEW_OUTPUT_WIDTH;
			            sInputBuffer.sCropInfo.nTop = 0;
			            sInputBuffer.sCropInfo.nWidth  = BIRDVIEW_SOURCE_SINGLE_WIDTH;
			            sInputBuffer.sCropInfo.nHeight = BIRDVIEW_SOURCE_SINGLE_HEIGHT;
		                break;
		            case BV_CHANNEL_LEFT:
		                sInputBuffer.bEnableCorp = 1;
			            sInputBuffer.sCropInfo.nLeft = 0;
			            sInputBuffer.sCropInfo.nTop = BIRDVIEW_SOURCE_SINGLE_HEIGHT;
			            sInputBuffer.sCropInfo.nWidth  = BIRDVIEW_SOURCE_SINGLE_WIDTH;
			            sInputBuffer.sCropInfo.nHeight = BIRDVIEW_SOURCE_SINGLE_HEIGHT;
		                break;
		            case BV_CHANNEL_RIGHT:
		                sInputBuffer.bEnableCorp = 1;
			            sInputBuffer.sCropInfo.nLeft = BIRDVIEW_SOURCE_SINGLE_WIDTH;
			            sInputBuffer.sCropInfo.nTop = BIRDVIEW_SOURCE_SINGLE_HEIGHT;
			            sInputBuffer.sCropInfo.nWidth  = BIRDVIEW_SOURCE_SINGLE_WIDTH;
			            sInputBuffer.sCropInfo.nHeight = BIRDVIEW_SOURCE_SINGLE_HEIGHT;
		                break;
            ...
            //pbuf->refCnt++;
			//ALOGD("onNextFrameHW ID %d refCnt %d",pbuf->index, pbuf->refCnt);
            memcpy(cam_buff->data, &buffer_type, 4);
			memcpy(cam_buff->data, &frame, 4);
...
bool CallbackNotifier::takePicture(const void* frame, bool is_continuous)
{
	...
	ALOGV("addrY: %x, src: %dx%d, pic: %dx%d, quality: %d, rotate: %d, Gps method: %s, \
		thumbW: %d, thumbH: %d, thubmFactor: %d, crop: [%d, %d, %d, %d] w=%d h=%d", 
        jpeg_enc.addrY,
        ...
        jpeg_enc.crop_y,
        jpeg_enc.crop_w,
		jpeg_enc.crop_h,mPictureWidth,mPictureHeight);
