...
android::HALCameraFactory  *pgEmulatedCameraFactory;

namespace android {

#define GET_CALLING_PID	(syscall(__NR_gettid))   //(IPCThreadState::self()->getCallingPid())
...
HALCameraFactory::HALCameraFactory(int ctype)
          ...
          mConstructedOK(false)
{
    ...
    for (int id = 0; id < MAX_NUM_OF_CAMERAS; id++)
    {
		...
		//if(((mCameraType == 0)&&((id==0)||(id==1)))||((mCameraType == 1)&&((id==4)||(id==5)||(id==6)||(id==7))))
		{
...
//def CAMERA_MANAGER_ENABLE
	if(mCameraType == 1){
		...
		for(int i=TVD_DEV_ID_START;i<TVD_DEV_ID_START+NB_CAMERA;i++)
		{
...
int HALCameraFactory::getCameraHardwareNum()
{
    ...
    else if (mAttachedCamerasNum == 0)
    {
        ...
        if (mRemovableCamerasNum == 1)
        {
            ...
            mHalCameraInfo[2].facing        = CAMERA_FACING_BACK;
            mHalCameraInfo[2].device_id     = 2;
            mHalCameraInfo[2].orientation   = orientation;
            mHalCameraInfo[2].fast_picture_mode = false;
            mHalCameraInfo[2].is_uvc        = true;
            mRemovableCamerasNum = 2;
            #endif
        }
...
#if 1
//def CAMERA_MANAGER_ENABLE
int HALCameraFactory::cameraDeviceOpen(int oview,int camera_id, hw_device_t** device)
{
    ...
    *device = NULL;
	...
	if((camera_id >= TVD_DEV_ID_START) && (camera_id < TVD_DEV_ID_START+NB_CAMERA))
	{
		if(oview)
		{
			for(i=TVD_DEV_ID_START;i<TVD_DEV_ID_START+NB_CAMERA;i++)
			{
		...
		else if (mHardwareCameras[camera_id-1] == NULL)
		{
			if(!mHardwareCameras[camera_id-1]->isCameraIdle()){
				LOGE("CAMERA[%d] is busy,wait for a moment",i);
				return -1;
			}
		}
	}
	...
	if(oview == 1)
	{
		...
		mCameraManager->setComposeOrder(0,1,2,3);
	...
	for(i= (camera_id +count -1);i >= camera_id;i--)
	{
		if (mHardwareCameras[i] == NULL)
			continue;
...
#if 0 //usb or cvbs do not need just for csi
...
{
    return pgEmulatedCameraFactory->getCameraHardwareNum();
}

int HALCameraFactory::get_camera_info(int camera_id,
                                           struct camera_info* info)
{
    return pgEmulatedCameraFactory->getCameraInfo(camera_id, info);
}
...
//def	CAMERA_MANAGER_ENABLE
	if(atoi(name) == 360)
	{
		return pgEmulatedCameraFactory->cameraDeviceOpen(1,TVD_DEV_ID_START, device);
	}
	else
	{
		return pgEmulatedCameraFactory->cameraDeviceOpen(0,atoi(name), device);
	}
#else
