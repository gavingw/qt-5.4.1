...
#define MAX_DI_SIZE  (720*576*3>>1)
#endif

//SONIX_CAMERA_H264 USED FOR 0 OR 1
#define SONIX_CAMERA_H264 1
//#define SUPPORT_VIRTUAL_CAMERA
...
class DoAdasThread : public Thread {
		...
		bool				mRequestExit;
    public:
        DoAdasThread(V4L2CameraDevice* dev) :
			Thread(false),
			mV4l2CameraDevice(dev),
			mRequestExit(false) {
		}
...
#ifdef USE_MP_CONVERT
	int g2d_scale(unsigned char *src, int src_width, int src_height, unsigned char *dst, int dst_width, int dst_height);
	int g2d_ops(int cmd ,unsigned char *src, int src_width, int src_height, unsigned char *dst, int dst_width, int dst_height);
	int g2d_flip(unsigned char *src, int src_width, int src_height, unsigned char *dst, int dst_width, int dst_height);
...
public:
	...
	void NV21Rotate(const void* nv21, void *dst, int width, int height);

#endif
#ifdef USE_Deinterlace_HW
	void Hw_Di_Process(unsigned char *presrc, int pre_width, int pre_height,
...
private:
    static status_t receiveDataFromCsiCamera(V4L2BUF_t *buf);
    static status_t sendDataToVirCamera(V4L2BUF_t *buf);
    static int mCsiFdsC2V[2];
    static bool mCsiRunning;
    static bool mCsiVirRunning;
