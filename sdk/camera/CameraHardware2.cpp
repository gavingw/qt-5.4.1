...
//#endif  fktodo
	mFrameData = (unsigned char*)malloc(2*1024*1024);
    mFrameFaceData.frameData = (unsigned char*)malloc(2*1024*1024);
...
void CameraHardware::initDefaultParameters()
{
	...
	{
		mCallbackNotifier.setExifModel("T3L");
	}
	...
	if ((mV4L2CameraDevice->getCameraType() != CAMERA_TYPE_CSI)|| (mCameraConfig->numberOfCamera() == 0))
	{
...
#if 1
//def CAMERA_MANAGER_ENABLE
	mV4L2CameraDevice->startPreview_d();
#endif
...
#if 1
//def CAMERA_MANAGER_ENABLE
	mV4L2CameraDevice->stopPreview_d();
#endif
	pthread_mutex_lock(&mCommandMutex);
...
void CameraHardware::stopRecording()
{
    ...
    ALOGD("(f:%s, l:%d) not need set mV4L2CameraDevice hwEncoder false, so delete it.", __FUNCTION__, __LINE__);
	//mV4L2CameraDevice->setHwEncoder(false);
...
status_t CameraHardware::setParameters(const char* p)
{
	...
	if(new_preview_width < new_video_width){
		if((new_video_width > 0 && new_video_height> 0) && (new_video_width < 4096 && new_video_height < 4096)){
...
status_t CameraHardware::setPreviewParam(struct view_info pvi,struct src_info  psi,bool tvout,int tvch)
{
	LOGV("%s", __FUNCTION__);
...
#if 1
//def CAMERA_MANAGER_ENABLE
	mV4L2CameraDevice->releaseCamera_d();
#endif
    cleanupCamera();
}
...
status_t CameraHardware::doStartPreview()
{
	...
	mParameters.getVideoSize(&video_width, &video_height);
	LOGV("%s getparams: video_width x video_height = %dx%d",
         __FUNCTION__, video_width, video_height);
	if((video_width > 0 && video_height> 0) && (video_width < 4096 && video_height < 4096)){
		mVideoCaptureWidth = video_width;
		mVideoCaptureHeight = video_height;
	}else{
		mVideoCaptureWidth = mCaptureWidth;
		mVideoCaptureHeight = mCaptureHeight;
	}
...
#ifdef ADAS_ENABLE
void CameraHardware::general_set_callbacks(struct camera_device* dev, camera_adas_data_callback adas_data_cb)
{
    CameraHardware* ec = reinterpret_cast<CameraHardware*>(dev->priv);
    if (ec == NULL) {
        LOGE("%s: Unexpected NULL camera device", __FUNCTION__);
	...
	ec->setAdasCallbacks(adas_data_cb);
}


int CameraHardware::set_adas_parameters(struct camera_device* dev, int key,int value)
{
    CameraHardware* ec = reinterpret_cast<CameraHardware*>(dev->priv);
    if (ec == NULL) {
        LOGE("%s: Unexpected NULL camera device", __FUNCTION__);
	...
	int ret = ec->setAdasParameters(key,value);	
    return ret;
}
#endif
...
int CameraHardware::set_waterMarkMultiple(struct camera_device *dev, const char* mWaterMark, int dispMode)
{
    CameraHardware *ec = reinterpret_cast<CameraHardware*>(dev->priv);
    if(ec == NULL){
        LOGE("%s: Unexpected NULL camera device", __FUNCTION__);
    ...
    int ret = ec->setWaterMarkMultiple((char* )mWaterMark,dispMode);
    //LOGV("%s-------%d",__FUNCTION__,ret);
    return ret;
}
//end watermark setting
#endif

//private flip setting
int CameraHardware::set_CameraFlipStatus(struct camera_device *dev, int flipEnable)
{
    LOGV("%s : flipEnable = %d",__FUNCTION__, flipEnable);
    CameraHardware* ec = reinterpret_cast<CameraHardware*>(dev->priv);
    if (ec == NULL) {
    ...
    return ec->setCameraFlipStatus(flipEnable);
}
//end
...
void CameraHardware::set_analog_input_color(struct camera_device* dev,int brightness,int contrast,int saturation)
{    
	CameraHardware* ec = reinterpret_cast<CameraHardware*>(dev->priv);    
	if (ec == NULL) 
	{        
	...
	ec->setAnalogInputColor(brightness,contrast,saturation);
}
int CameraHardware::set_previewparam(struct camera_device *dev, struct view_info pvi,struct src_info  psi,bool tvout,int tvch)
{
    CameraHardware* ec = reinterpret_cast<CameraHardware*>(dev->priv);
    if (ec == NULL) {
        LOGE("%s: Unexpected NULL camera device", __FUNCTION__);
    ...
    return ec->setPreviewParam(pvi,psi,tvout,tvch);
...
//    CameraHardware::set_preview_window,
...
#ifdef ADAS_ENABLE
    CameraHardware::general_set_callbacks,
    CameraHardware::set_adas_parameters,
#endif
    ...
    CameraHardware::dump,
    CameraHardware::set_analog_input_color,
    CameraHardware::set_fd,
    CameraHardware::set_CameraFlipStatus,
#ifdef WATERMARK
    CameraHardware::set_waterMarkMultiple,
#endif
	CameraHardware::set_previewparam,
};
